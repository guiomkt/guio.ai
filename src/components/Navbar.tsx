        ...
        <div className="flex flex-col space-y-6 items-center">
          {menuItems.map((item, index) => (
            <a 
              key={index} 
              href={item.href} 
              className="text-guio-white hover:text-guio-red transition-colors duration-200 text-xl font-medium py-2"
              onClick={(e) => {
                e.preventDefault();
                handleMenuItemClick(item.href);
              }}
          ...
          <Button 
            className="bg-guio-red hover:bg-guio-red/80 text-white w-full mt-4" 
            onClick={handleContactClick}
          >
            <MessageCircle className="mr-2 h-4 w-4" />
            Converse com um agente GUIO
